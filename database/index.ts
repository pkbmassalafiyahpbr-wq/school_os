...
export function seedData() {
  ...
  for (const name of studentNames) {
    ...
    database.prepare('UPDATE students SET user_id = ? WHERE full_name = ?').run(id, name)
  }
  ...
  const insertNotif = database.prepare('INSERT INTO notifications (title, message, target_role) VALUES (?, ?, ?)')
  ...
  const attempts = database.prepare('SELECT qa.id, qa.student_id, qa.quiz_id, qa.score, q.teacher_id, q.subject_id FROM quiz_attempts qa JOIN quizzes q ON qa.quiz_id = q.id WHERE qa.finished_at IS NOT NULL').all() as any[]
  ...
  insertAudit.run(1, 'Login', 'User', 1); insertAudit.run(2, 'Buat Materi', 'Material', 1); insertAudit.run(3, 'Buat Quiz', 'Quiz', 1); insertAudit.run(5, 'Generate Data Demo', 'System', 0)
}
